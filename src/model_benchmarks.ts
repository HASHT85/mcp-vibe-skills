...
export function formatBenchmarksForPrompt(): string {
    return MODEL_BENCHMARKS.sort((a, b) => b.coding - a.coding)
        .map(
            (b) =>
                `- ${b.id} | provider: "${b.provider}" | Coding: ${b.coding}/100 | Agentic: ${b.agentic}/100 | Intelligence: ${b.intelligence}/100 | Speed: ${b.speed} | Best for: ${b.bestFor.join(", ")}`
        )
        .join("\n");
}
