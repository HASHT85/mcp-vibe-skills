...
export function DetailPanel({ pipeline, liveEvents, open, onClose }: DetailPanelProps) {
    ...
    const getAgentStatusStyle = (status: string) => {
        switch (status) {
            case 'active': return 'border-v-accent/40 bg-accent-subtle';
            case 'done': return 'border-status-success/30 bg-status-success/5';
            case 'error': return 'border-status-error/30 bg-status-error/5';
            default: return 'border-surface-6 bg-surface-3';
        }
    };
    ...
    const getEventTypeColor = (type: string) => {
        switch (type) {
            ...
            case 'deploy': return 'text-v-accent';
                        ...
                        <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                                ...
                                <span className={`text-2xs font-medium ${
                                    pipeline.phase === 'COMPLETED' ? 'text-status-success' :
                                    pipeline.phase === 'FAILED' ? 'text-status-error' : 'text-v-accent'
                                }`}>
                                    {pipeline.phase}
                                </span>
                            </div>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-3">
                        {tab === 'agents' && (
                            <div className="flex flex-col gap-2">
                                ...
                                <div className="grid grid-cols-3 gap-2 mb-2">
                                    ...
                                    <div className="bg-surface-3 rounded-lg p-3 text-center">
                                        <div className="text-lg font-bold text-status-success">{completedAgents.length}</div>
                            ...
                            <div className="flex flex-col gap-1">
                                {pipelineEvents.length === 0 ? (
                                    <div className="py-8 text-center">
                                        <span className="material-symbols-outlined text-2xl text-text-muted mb-2 block">receipt_long</span>
                                        <p className="text-xs text-text-tertiary">No events yet</p>
                                    </div>
                                ) : (
                                    pipelineEvents.map((event, i) => (
                                        <div key={i} className="flex gap-2.5 py-2 px-2 rounded-lg hover:bg-surface-3/50 transition-colors">
                                            <span className="text-sm shrink-0">{event.agentEmoji || '📡'}</span>
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2 mb-0.5">
                                                    <span className={`text-2xs font-semibold uppercase ${getEventTypeColor(event.type)}`}>
                        ...
                        {tab === 'info' && (
                            <div className="flex flex-col gap-3">
                                ...
                                {pipeline.github && (
                                    <InfoRow label="GitHub" value={pipeline.github.url} link />
                                )}
                                {pipeline.description && (
                                    <div className="bg-surface-3 rounded-lg p-3 mt-2">
                                        <span className="text-2xs text-text-tertiary font-medium uppercase tracking-wider block mb-1.5">Description</span>
                                        <p className="text-xs text-text-secondary leading-relaxed">{pipeline.description}</p>
                                    </div>
                                )}
                                {pipeline.tokenUsage && (
                                    <div className="bg-surface-3 rounded-lg p-3">
                                        <span className="text-2xs text-text-tertiary font-medium uppercase tracking-wider block mb-2">Token Usage</span>
                                        <div className="flex gap-4">
                                            <div>
                                                <div className="text-sm font-semibold text-v-accent">{(pipeline.tokenUsage.inputTokens || 0).toLocaleString()}</div>
                                                <div className="text-2xs text-text-muted">Input</div>
                                            </div>
                                            <div>
                                                <div className="text-sm font-semibold text-v-alert">{(pipeline.tokenUsage.outputTokens || 0).toLocaleString()}</div>
                                ...
                                {pipeline.error && (
                                    <div className="bg-status-error/10 rounded-lg p-3 border border-status-error/20">
                                        <span className="text-2xs text-status-error font-medium uppercase tracking-wider block mb-1">Error</span>
                                        <p className="text-xs text-status-error/80 leading-relaxed break-words">{pipeline.error}</p>
                                    </div>
                                )}
                            </div>
                        )}
...
function InfoRow({ label, value, mono, link }: { label: string; value: string; mono?: boolean; link?: boolean }) {
    return (
        <div className="flex items-center justify-between py-1.5 border-b border-surface-6/30 last:border-0">
            ...
            {link ? (
                <a href={value} target="_blank" rel="noopener noreferrer" className="text-xs text-v-accent hover:underline truncate max-w-[200px]">
                    {value}
                </a>
            ) : (
                <span className={`text-xs text-text-primary truncate max-w-[200px] ${mono ? 'font-mono' : ''}`}>{value}</span>
            )}
        </div>
    );
}
