...
export function QuickDeployView() {
    ...
    const handleDeploy = async () => {
        ...
        try {
            ...
            secrets.forEach((s) => {
                if (s.key && s.value) secretsObj[s.key] = s.value;
            });
        ...
        >
            ...
            <AnimatePresence mode="wait">
                    ...
                    >
                        <div className="bg-[#0B0F14] border-2 border-v-accent/30 p-8">
                            ...
                            <div className="mt-6 text-[10px] text-slate-600 leading-relaxed">
                                Paste a GitHub repository URL. VEIST will clone it, read the README, detect
                                Dockerfile/docker-compose, and prepare deployment to your Hostinger VPS with Traefik
                                SSL.
                            </div>
                        </div>
                    </motion.div>
                            ...
                            <div className="grid grid-cols-2 gap-3 md:gap-4">
                                <div className="bg-black/50 border border-slate-800 p-3">
                                    ...
                                    <div className="text-sm font-bold text-white">
                                        {LANG_ICONS[analysis.language]} {analysis.language.toUpperCase()}
                                    </div>
                                </div>
                                <div className="bg-black/50 border border-slate-800 p-3">
                                    <div className="text-[9px] text-slate-500 uppercase tracking-widest mb-1">
                                    ...
                                    <div className="text-sm font-bold text-white">
                                        {analysis.dockerHubImage ? "✅ Found" : "—"}
                                    </div>
                                    {analysis.dockerHubImage && (
                                        <div className="text-[10px] text-v-accent mt-1 truncate">
                                            {analysis.dockerHubImage}
                                        </div>
                                    )}
                                </div>
                                    ...
                                    {Object.entries(MODE_LABELS).map(([mode, info]) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {!isAvailable && (
                                                    <div className="text-[8px] text-red-500/50 mt-1">NOT AVAILABLE</div>
                                                )}
                                            </button>
                                        );
                                    })}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        value={projectName}
                                        onChange={(e) =>
                                            setProjectName(e.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, "-"))
                                        }
                                        spellCheck="false"
                                    />
                                </div>
                            ...
                            <div className="flex flex-col gap-2">
                                {secrets.map((s, i) => (
                                    <div key={i} className="flex gap-2 items-center">
                                        ...
                                        >
                                            <span className="material-symbols-outlined text-[14px]">
                                                {secretsVisible.has(i) ? "visibility_off" : "visibility"}
                                            </span>
                                        </button>
                                        <button
                ...
                {step === "deploying" && (
                    ...
                    >
                        <div className="bg-[#0B0F14] border-2 border-v-accent/30 p-12 text-center">
                            ...
                            <h2 className="text-xl font-black text-white tracking-widest uppercase mb-4">
                                DEPLOYING...
                            </h2>
                            ...
                            <p className="text-slate-500 text-xs">
                                Status:{" "}
                                <span className={deployState === "error" ? "text-red-400" : "text-v-accent"}>
                                    {deployState || "SENT"}
                                </span>
                            </p>
                ...
                {step === "done" && (
                    ...
                    >
                        <div
                            className={`bg-[#0B0F14] border-2 p-8 ${deployState === "success" ? "border-green-500/50" : "border-red-500/50"}`}
                        >
                            <div className="text-center mb-6">
                                <span
                                    className={`material-symbols-outlined text-5xl ${deployState === "success" ? "text-green-400" : "text-red-400"}`}
                                >
                                        ...
                                        <div className="bg-black/50 border border-slate-800 p-4">
                                            ...
                                            {containers.map((c: any) => (
                                                <div key={c.id} className="flex items-center gap-2 text-xs text-white">
                                                    <span
                                                        className={`w-2 h-2 rounded-full ${c.state === "running" ? "bg-green-400" : "bg-red-400"}`}
                                                    />
                                                    <span className="font-mono">{c.name}</span>
                                                    <span className="text-slate-500">{c.status}</span>
                                                    {c.health && (
                                                        <span className="text-green-400 text-[10px]">[{c.health}]</span>
                                                    )}
                                                </div>
                                            ))}
                                        </div>
