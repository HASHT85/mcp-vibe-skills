...
export function ProjectCard({ pipeline: p, onClick, onRetry }: ProjectCardProps) {
    ...
    const badgeLabel = p.phase || "STANDBY";
    ...
    return (
        ...
        >
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
                <h2
                    className="text-sm font-bold leading-tight font-headline tracking-tight truncate pr-2 text-on-surface group-hover:text-[#D7FF2F] transition-colors uppercase"
                    title={formattedName}
                >
                    {formattedName}
                </h2>
                <span
                    className={`${statusBg} ${statusText} ${statusBorder} border px-3 py-1 text-[10px] font-mono uppercase tracking-widest shrink-0`}
                >
                    {badgeLabel}
                </span>
            </div>
